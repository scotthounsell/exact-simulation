...
from poisson_process_old import Poisson_Process
...
class Driftless_Exact:
    ...
    def get_antithetic_malliavin(self):
        c2 = self.sigma_func_deriv(self.X[-2])
        if c2 == 0:
            self.X_bar = self.X[-2] - self.sigma_func(self.X[-2]) * self.dW[-2]
        else:
            c1 = self.sigma_func(self.X[-2]) - c2 * self.X[-2]
            self.X_bar = -c1/c2 + (c1/c2 + self.X[-2]) * exp(-0.5*(c2**2)*self.poisson.dt[-1] - c2*self.dW[-1])
            
        a = 0.5*(self.sigma_func(self.X[-2]))**2
        sigma_tilde = self.sigma_func(self.X[-3]) + self.sigma_func_deriv(self.X[-3])*(self.X[-2]-self.X[-3])
        a_tilde = 0.5*sigma_tilde**2
        dW = self.dW[-1] # k or k+1
        dt = self.poisson.dt[-1] # k or k+1
        return (a-a_tilde) / (2*a) * (self.sigma_func_deriv(self.X[-2])*dW/dt + (dW**2 - dt)/dt**2)
        
    def antithetic_psi(self, func):
        """
        Calculate antithetic psi.
        E[psi] = E[g(X_T)] where X_T is the last step of the standard Euler scheme process.
        """
        ...
        product = 1
        for k in xrange(1, N_T):
            print k, self.malliavin_weight(k)
            product *= self.malliavin_weight(k)
            
        product *= self.get_antithetic_malliavin()
        
        return exp(self.beta*self.T) * (func(self.X_bar) - func(self.X[-2])*(N_T > 0)) * self.beta**(-N_T) * product
        
    def get_average_psi(self, func):
        return 0.5 * (self.psi(func) + self.antithetic_psi(func))       
...
for i in xrange(N):
    ...
    results[i] = test.get_average_psi(g)
    
#plt.show()

print "results", results
print "mean results", np.mean(results)
print "std results", np.std(results)
