...
Driftless_Exact <- function(num_of_paths, beta, sigma, sigma_deriv, T=1, X0=0){
    ...
    run_monte_carlo <- function(g_maker, strikes, num_paths_to_plot=FALSE){
        ...
        #     num_paths_to_plot (Optional[int or boolean]): How many paths should be plotted. Defaults to FALSE.  If TRUE, defaults to 100.
        # Returns:
        ...
        lineCols <- rainbow(num_paths_to_plot)
        for (path in 1:num_of_paths){
            # simulate_process()
            # Simulates the process Xhat across time at steps determined by a Poisson process.
            ...
            dW <- rnorm(length(poisson$dt), sd=sqrt(poisson$dt)) # Including 1st element of dt (zero) ensures a zero in front to match reference paper indices
            ...
            # Using initializer=1 to protect against the edge case of no arrivals before T
            ...
            if (plotted_paths < num_paths_to_plot){ # Plots the first num_paths_to_plot paths.
                if (plotted_paths==0){ # First plot does labeling
                    plot(poisson$t, Xhat, main='Generated Paths', xlab='t', ylab=expression(hat('X')[t]), ylim=X0+sqrt(T)*c(-1,1), type='l', col=lineCols[plotted_paths+1])
                } else {
                    lines(poisson$t, Xhat, col=lineCols[plotted_paths+1])
                }
