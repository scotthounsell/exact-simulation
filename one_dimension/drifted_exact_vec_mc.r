...
Drifted_Exact_VecMC <- function(num_of_paths, beta, mu, sigma0=1, Texp=1, X0=0, convert_y_to_x=function(y) y, convert_x_to_y=function(x) x){
    ...
    run_monte_carlo <- function(g_maker, strikes, num_paths_to_plot=FALSE){
        ...
        # Using mclapply from the 'parallel' library for multithreading (overhead might only outweighed for all_dt, unclear)
        ...
        all_N_T <- as.numeric(lapply(all_dt, length)) - 2 # N_T is the number of default events (so last arrival and starting time don't count towards N_T)

        sumPrices <- function(N_T){
        ...
        return(rowSums(vapply(unique(all_N_T), sumPrices, FUN.VALUE=strikes)) / num_of_paths)
    }
