...
Drifted_Exact <- function(num_of_paths, beta, mu, sigma0=1, Texp=1, X0=0, convert_y_to_x=function(y) y, convert_x_to_y=function(x) x){
    ...
    run_monte_carlo <- function(g_maker, strikes, num_paths_to_plot=FALSE){
        ...
        num_paths_to_plot <- min(num_paths_to_plot, num_of_paths) # Cannot plot more than we have
        ...
        sum_prices <- function(N_T){
            ...
            # Initialize Yhat with Y0 in first column (more efficient to just initialize all values as Y0)
            ...
            if (num_paths_to_plot){
                ...
                if (plot_num == 1){ # First plot initializes the figure
            ...
            # This method may be slower, but easier to understand: just return the price matrix
        ...
        # This method may be slower, but easier to understand: just return the prices (psi_matrix)
